---
title: Memory structure
description: "relay uses two memory layers: ephemeral session memory in SQLite and persistent project memory in .relay/memory/ Markdown files — shared across all agents and sessions."
---

Agent memory in relay is split into two layers: **session memory** and **project memory**.

## File structure

```
my-project/
└── .relay/
    ├── memory/                  # persistent memory across sessions (recommended to git-commit)
    │   ├── project.md           # architecture, domain, and tech stack summary
    │   ├── lessons.md           # repeated mistakes, important decision history
    │   └── agents/
    │       ├── pm.md            # PM personal memory
    │       ├── fe.md            # FE: component structure, conventions
    │       ├── be.md            # BE: API patterns, DB schema
    │       ├── da.md            # DA: event / metrics taxonomy
    │       ├── designer.md      # Designer: UI patterns, design system
    │       ├── qa.md            # QA: recurring bug patterns, fragile areas
    │       └── deployer.md      # Deployer: deployment history, cautions
    └── sessions/                # per-session summaries (gitignored; auto-generated)
        ├── 2026-03-13-001/
        │   └── summary.md
        └── ...
```

## Session memory (SQLite)

Within-session data is stored in SQLite:
- Messages (`messages` table)
- Tasks (`tasks` table)
- Artifacts (`artifacts` table)
- Reviews (`reviews` table)
- WebSocket events (`events` table)

When a session ends, `save_session_summary` writes a human-readable summary to `.relay/sessions/{session_id}/summary.md`. Full task and message history is already persisted in SQLite. The sessions directory is gitignored — it is ephemeral and local-only.

## Project memory (Markdown files)

Memory that persists across sessions is stored as Markdown files in `.relay/memory/`.

### project.md

The project-wide summary written by the coordinator during `/relay:init`:
- Domain and key features
- Architecture overview
- Team conventions

### lessons.md

Shared team retrospective and decision history:
- Recurring mistake patterns
- Important technical decisions
- Areas that need extra care

### agents/&#123;agentId&#125;.md

Each agent's personal memory:
- Role-specific important code patterns
- Conventions discovered in previous sessions
- Cautions for specific areas

## Memory MCP tools

### Reading

```typescript
// Read an agent's personal memory
read_memory({ agent_id: "fe" })
// → returns .relay/memory/agents/fe.md

// Read project memory (omit agent_id)
read_memory({})
// → returns combined project.md + lessons.md
```

### Writing (section overwrite)

```typescript
write_memory({
  agent_id: "fe",
  key: "component-patterns",
  content: "Button components always use shadcn/ui as the base..."
})
// replaces the ## component-patterns section in .relay/memory/agents/fe.md
```

### Appending (cumulative)

```typescript
// Append to an agent's personal memory
append_memory({
  agent_id: "qa",
  content: "2026-03-13: found missing empty-email case in login flow"
})

// Append to team memory (lessons.md) — omit agent_id
append_memory({
  content: "Always confirm BE auth header spec before FE implementation starts"
})
```

## Session lifecycle

### On session start

Automatically injected into each agent's system prompt:
1. `project.md` + `lessons.md` (project memory)
2. `agents/{agentId}.md` (personal memory)

### On session end

1. Each agent saves new discoveries via `write_memory` or `append_memory`
2. Team retrospective is appended to `lessons.md`
3. `save_session_summary` archives the session

## Team sharing

It is recommended to commit `.relay/memory/` files to git so the whole team shares them:

```bash
git add .relay/memory/
git commit -m "chore: update relay memory"
```

This lets new team members and new Claude Code sessions immediately benefit
from the context of previous sessions.

## RELAY_DIR environment variable

The default memory path is `{cwd}/.relay`, but it can be changed via the `RELAY_DIR` environment variable:

```bash
RELAY_DIR=/path/to/shared/.relay bun run start
```
