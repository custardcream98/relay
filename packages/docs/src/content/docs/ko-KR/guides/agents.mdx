...
| `tags` | 아니오 | `/relay:relay`의 스마트 팀 매칭에 쓰이는 분류 태그 |
| `language` | 아니오 | 응답 언어 강제 (예: `"Korean"`, `"English"`) |
| `disabled` | 아니오 | `true`로 설정하면 레지스트리에서 제외 |
...
`extends`는 두 번째 패스에서 처리하므로 YAML 파일 내 선언 순서는 상관없습니다. 세션 파일의 에이전트도 풀 에이전트 ID를 `extends`로 지정할 수 있습니다.

## tags 필드

`tags`는 `/relay:relay`가 팀을 제안할 때 매칭에 활용합니다. 에이전트의 도메인과 기술을 설명하는 태그를 추가하세요:
...
`tools` 배열에 없는 MCP 도구는 호출할 수 없습니다.
