...
**Agent Arena** (좌) — 세션의 모든 에이전트와 실시간 상태(`idle` / `working` / `waiting`), 현재 생각의 스니펫을 보여줘요. 클릭하면 Activity Feed를 해당 에이전트 기준으로 필터링해요.

**Activity Feed** (우상단) — 모든 이벤트를 실시간 타임라인으로 표시해요: 메시지, 태스크 업데이트, 아티팩트, 리뷰 요청, 에이전트 추론 스트림. 이벤트 타입과 에이전트로 필터 가능해요. **Messages** 탭으로 전환하면 에이전트 간 대화를 Slack 스레드 형식으로 볼 수 있어요.

**Task Board** (하단, 접을 수 있음) — `todo → in_progress → in_review → done` Kanban. 태스크 클릭 시 상세 모달이 열려요.
