...
- **프론트엔드**: React + Vite (별도 `dashboard/` 패키지)
- **스타일**: Tailwind CSS
...
- `src/agents/loader.ts`가 merge 처리

### 설치 방식 (글로벌/로컬)
- 글로벌: `~/.claude/skills/`에 스킬 설치 + `claude mcp add --global`
- 로컬: `.claude/skills/`에 스킬 설치 + `claude mcp add`
- 로컬이 글로벌을 오버라이드
...
│   └── sessions.ts       # list_sessions, save_session_summary
├── agents/
