...
**Activity Feed** (우상단) — 모든 이벤트를 실시간 타임라인으로 표시해요: 메시지, 태스크 업데이트, 아티팩트, 리뷰 요청, 에이전트 추론 스트림. 이벤트 타입과 에이전트로 필터 가능해요. 키보드 단축키(`j`/`k`/`Enter`/`Escape`)로 탐색할 수 있어요. 아티팩트 카드를 클릭하면 전체 내용을 확인할 수 있어요.
